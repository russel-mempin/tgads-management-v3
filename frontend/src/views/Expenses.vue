...
import { getAllExpenses, getAllTodayExpenses, archiveExpense } from '@/api/expenses';
...
const editingData = ref<ExpenseCreate | null>(null)

const fetchExpenses = async () => {
	if (authStore.isOwner) {
		expensesList.value = await getAllExpenses()
	}
	expensesList.value = await getAllTodayExpenses()
}
onMounted(async () => {
	fetchExpenses()
});
...
<template>
	<ExpenseForm v-model:is-visible="isVisible" @saved="fetchExpenses" :edit-item="editingData"
		@update:is-visible="onClose" />
	<ConfirmDialog>
		<template #icon>
	...
	<div class="mx-12 my-6 flex flex-col h-full min-h-0 overflow-hidden">
		...
		<section class="mt-6 flex-1 h-full rounded-md border border-slate-300 overflow-hidden bg-white">
			<DataTable scrollable scroll-height="flex" :pt="{ root: 'h-full' }" :value="expensesList" dataKey="id"
				...
				<Column field="account_name" header="Fund Source"></Column>

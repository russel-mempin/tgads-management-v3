...
import { Dialog, DatePicker, InputNumber, InputText, Button, useToast } from 'primevue';
import { createMiscSale, updateMiscSale } from '@/api/sales';
import { nowInManila } from '@/utils/formatters';

const toast = useToast();
const props = defineProps<{
	isVisible: boolean
	editItem?: MiscSale | null
}>()

const emit = defineEmits<{
	(e: 'update:isVisible', value: boolean): void
	(e: 'saved'): void
	(e: 'update-item', value: MiscSale): void
}>()

const item = ref<MiscSale>({
	date: nowInManila(),
	amount: 1,
	description: '',
})
...
const onSave = async () => {
	try {
		if (props.editItem) {
			...
			await createMiscSale(item.value)
		}
...
<template>
	<Dialog :visible="isVisible" @update:visible="emit('update:isVisible', $event)" modal
		:header="editItem ? 'Edit Misc Sale' : 'Add Misc Sale'" :style="{ width: '40rem' }">

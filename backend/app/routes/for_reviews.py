...
@router.get("/{for_review_id}", response_model=ForReviewDetails)
def read_for_review_details(for_review_id: uuid.UUID, db: Session = Depends(get_session)):
    return get_for_review_details(db, for_review_id)
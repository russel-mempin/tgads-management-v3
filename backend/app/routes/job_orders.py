...
from app.enums import UserRoles, SizeUnit
import uuid
...
def compute_unit_price(
    ...
    return get_price(db, height=height, width=width, service_name=service_name, size_unit=size_unit)
...
def archive(jo_number: int, db: Session = Depends(get_session), current_user: User = Depends(get_current_active_user)):
    return archive_job_order(db, jo_number, current_user.id)

@router.put("/{jo_number}")
def update(jo_number: int, data: JobOrderCreate, db: Session = Depends(get_session), current_user: User = Depends(get_current_active_user)):

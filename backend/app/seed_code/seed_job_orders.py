...
from app.models import (
    Service, ServiceOption, Customer, JobOrder, JobItem,
    ExtraService, JobItemExtra, VoidJobOrder
)
            ...
            if customer_name.lower() == "cancelled":
                if jo_number_raw:
                    existing_void = session.exec(
                        select(VoidJobOrder).where(VoidJobOrder.jo_number == int(jo_number_raw))
                    ).first()
        ...
        with open(file_path, newline="") as f:
            for row in csv.DictReader(f):
                ...
                if jo_number_raw.isdigit():
                    ...
                    skipped.append(f"'{jo_number_raw}': not a real JO number and not "
                ...
                if job_order is None:
                    skipped.append(f"JO {jo_number}: no matching JobOrder " f"(Check if it is cancelled.)")
                    continue
 
                service = session.exec(select(Service).where(Service.name == row["service"].strip())).first()
                if service is None:
                    skipped.append(f"JO {jo_number}: no Service named '{row['service']}'")
                    continue
                option = session.exec(
                ...
                height = to_float(row["height"]) if row.get("height", "").strip() else None
                size_unit = SizeUnit(row["unit"].strip()) if row.get("unit", "").strip() else None
                quantity = to_int(row["quantity"]) if row.get("quantity", "").strip() else 1
                unit_price = parse_currency(row.get("unit_price", "")) if has_price else 0.0
                ...
                seq = item_seq_by_jo_and_abbr.get((jo_number, service.abbreviation), 0) + 1
                item_seq_by_jo_and_abbr[(jo_number, service.abbreviation)] = seq
                if seq > 999:
                    skipped.append(f"JO {jo_number}: more than 999 '{service.abbreviation}' "
                ...
                if not (has_size and has_price):
                    skipped.append(f"JO {jo_number} ({service.name}): missing size/price ")
                ...
                if extra_service_name:
                    extra_service = session.exec(select(ExtraService).where(ExtraService.name == extra_service_name)).first()
                    extra_qty = int(row["extra_quantity"]) if row.get("extra_quantity", "").strip() else 1
                    price_snapshot = extra_charge if row.get("extra_charge", "").strip() else extra_service.price
                    session.add(JobItemExtra(
                        job_item_id=item.id,

...
def get_payment_for_review_details(db: Session, entity_id: uuid.UUID) -> ForReviewDetails:
    for_review_item = db.exec(
        select(ForReview)
        .where(ForReview.entity_id == entity_id)
        .options(
    ...
    if not for_review_item.entity_type == ReviewEntityType.PAYMENT:
        raise HTTPException(
            status_code=status.HTTP_400_NOT_FOUND,
    ...
    if not entity:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Unlinked payment not found."
        )
